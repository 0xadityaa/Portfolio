...
In this phase, JS engine executes the code line by line. It starts with the first line of code and executes it one by one. It now updates the initially reserved memory whih was set to `undefined` with the actual value of the variable.

When the code exicution reaches to the function call, a new exicution context is created inside of current context.
...
Now that we have an understanding of the brutforce way fo JS execution, lets look at how it is executed in a more practical way by the JS engine.
...
The call stack also has a core component called `Global Execution Context`. This is the context that is used to execute the code outside of any function. It is the initial context that is created when the code is executed first. After complition of every function call in the stack, controll of execution returns to the Global Execution Context which then runs further code.
